...
class StandaloneSingleSpaPlugin {
    ...
    return `
      System.import('single-spa').then(function (singleSpa) {
        singleSpa.registerApplication({
          ...
          customProps : ${JSON.stringify(this.options.customProps)}
        });
  ...
  parcelRuntime() {
    const customPropsString =
      this.options.customProps &&
      Object.entries(this.options.customProps)
        .map(([key, value]) => `'${key}': '${value}'`)
        .join(",");
    return `
      Promise.all([System.import('single-spa'), System.import('${
        ...
        singleSpa.mountRootParcel(parcelConfig, { domElement: parcelContainer, name: '${
          this.options.appOrParcelName
        }' ${customPropsString ? ", " + customPropsString : ""}});
      });
    `;
  }
